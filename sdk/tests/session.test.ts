...
const mockSetPlayerName = vi.fn((name: string) => {
  mockPlayerName = name;
  mockIsAutoGenerated = false;
});
const mockGetOrCreatePlayerName = vi.fn(() => {
  if (mockPlayerName === null) {
    mockPlayerName = 'BOLDFALCON';
    mockIsAutoGenerated = true;
  }
  return mockPlayerName;
});
const mockIsAutoGeneratedName = vi.fn(() => mockIsAutoGenerated);
...
vi.mock('../src/PlayerIdentity', () => ({
  PlayerIdentity: function() {
    return {
      ...
      getOrCreatePlayerName: mockGetOrCreatePlayerName,
      isAutoGeneratedName: mockIsAutoGeneratedName,
    };
  },
}));
...
describe('KeeperBoardSession', () => {
  ...
  describe('identity management', () => {
    ...
    it('should auto-generate a name when no name is stored', () => {
      mockPlayerName = null;
      const session = new KeeperBoardSession({
        apiKey: 'test-key',
        leaderboard: 'main',
      ...
      expect(name).toBe('BOLDFALCON'); // Mock auto-generated name
      expect(mockGetOrCreatePlayerName).toHaveBeenCalled();
    });
    ...
    it('hasExplicitPlayerName() should return false when name is auto-generated', () => {
      mockPlayerName = 'BOLDFALCON';
      mockIsAutoGenerated = true;
      ...
      expect(session.hasExplicitPlayerName()).toBe(false);
    });
    ...
    it('hasExplicitPlayerName() should return true after setPlayerName() is called', () => {
      ...
      session.setPlayerName('CustomName');
      mockPlayerName = 'CustomName';
      mockIsAutoGenerated = false;

      expect(session.hasExplicitPlayerName()).toBe(true);
    });

    it('hasExplicitPlayerName() should return false when no name stored', () => {
      mockPlayerName = null;
      mockIsAutoGenerated = false;
  ...
  describe('submitScore()', () => {
    ...
    it('should use auto-generated name when no explicit name set', async () => {
      ...
      expect(mockSubmitScore).toHaveBeenCalledWith({
        playerGuid: 'test-player-guid',
        playerName: 'BOLDFALCON', // Auto-generated by mock
        score: 1000,
        metadata: undefined,
      });
    });
